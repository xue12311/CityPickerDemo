...
import com.jl.city_picker.R;
import com.jl.city_picker.adapter.decoration.GridItemDecoration;
...
import com.jl.city_picker.util.WeakHandler;
...
public class CityListAdapter extends RecyclerView.Adapter<CityListAdapter.BaseViewHolder> {
    ...
    public CityListAdapter(Context context, List<City> data, List<HotCity> hotData, int state, Boolean isEnabledLocatedCity) {
        this.mData = data;
        ...
        for (int i = 0; i < size; i++) {
            if (TextUtils.equals(index.substring(0, 1), mData.get(i).getSection().substring(0, 1))) {
                if (mLayoutManager != null) {
                    ...
                    if (i == 0) {//热，定，A
                        //防止滚动时进行刷新
                        new WeakHandler(mContext.getMainLooper()).postDelayed(() -> {
                            if (stateChanged) {
                                notifyItemChanged(0);
                            }
                        }, 1000);
    ...
    public void onBindViewHolder(@NonNull BaseViewHolder holder, int position) {
        ...
        if (holder instanceof DefaultViewHolder) {
            ...
            ((DefaultViewHolder) holder).name.setText(data.getName()+"("+data.getProvince()+")");
            ((DefaultViewHolder) holder).name.setOnClickListener(view -> {
            ...
            if (holder instanceof LocationViewHolder) {
                ...
                ((LocationViewHolder) holder).container.setOnClickListener(view -> {
    ...
    public int getItemViewType(int position) {
        if (isEnabledLocatedCity) {//启用定位城市
            //定位城市
            if (position == 0 && TextUtils.equals(AppConstant.LocatedCityFirstWordName, mData.get(position).getSection().substring(0, 1))) {
                return VIEW_TYPE_LOCATION;
            }
            //热门城市
            if (position == 1 && TextUtils.equals(AppConstant.HotCityFirstWordName, mData.get(position).getSection().substring(0, 1))) {
                return VIEW_TYPE_HOT;
            }
        } else {//不启用定位城市
            if (position == 0 && TextUtils.equals(AppConstant.HotCityFirstWordName, mData.get(position).getSection().substring(0, 1))) {
                return VIEW_TYPE_HOT;
            }
        }
